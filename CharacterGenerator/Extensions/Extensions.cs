	...
	public static class Extensions
	{
		...
		public static T RandomPickUp<T>(this List<T> collection) where T : ImageMetadata
		{
			if(collection.Count == 0)
			{
				return default(T);
				throw new Exception();
				//collection.Add(nullFactory());
				//return default(T);
			}
			var randomIndex = random.Next(0, collection.Count);
			//if (collection.Count > 1 && collection.Any(x => x.Id.ToLower().Contains("empty")))
			//{
			//	randomIndex = random.Next(0, collection.Count - 1);
			//}
			//else
			//{
		...
		//public static T RandomPickUp<T>(this List<T> collection)
		//{
